...
public class Capitalist {  // 资本家
	...
	void handleProduces(Society society)  // 倒计时，结束已到时间的生产，产品收入库存，回收设备，计算设备损耗，归还劳动力，支付工人工资
	{
		Produce p1, p2;
	...
	void handleLoans(Society society)  // 倒计时，回收已到期的贷款
	{
		Loan lo1, lo2;
	...
	void invest(Society society, Random ra)  // 投资
	{
		double d1,   // 工业生产
				d2,   // 借贷
				d3,   // 投机
		...
		if (d4 != 0)  // 有利可图
		{
			...
			{  // System.out.println("工业生产投资："+d1);
				...
				for (; c * i1 + society.wor.salary(i1 * pro1.workers_number) * i1 * pro1.workers_number
						* pro1.pro_time < d1; i1++)
					;  // 根据劳动力价格调整开动设备数
				for (; (c * i1
						+ society.wor.salary(i1 * pro1.workers_number) * i1 * pro1.workers_number * pro1.pro_time > d1
						|| i1 > avaEqu_number) && i1 > 0; i1--)
					;  // 根据劳动力价格调整开动设备数
				  // System.out.println(" 开动设备数："+i1);
				capital -= c * i1
						+ society.wor.salary(i1 * pro1.workers_number) * i1 * pro1.workers_number * pro1.pro_time;  // 将资本用于进行生产
				...
				prod = new Produce(i1 * pro1.equ_productivity, i1, i1 * pro1.workers_number,
						society.wor.salary(i1 * pro1.workers_number), pro1.pro_time, prod);
				/* 头插进行中的生产链表 */}
