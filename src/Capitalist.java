...
public class Capitalist {  // 资本家
	...
	void initialize(Society society, Random ra)  // 随机初始化
	{
		...
		if (pro_participant) {
			do
				pro_id = ra.nextInt(society.productKinds);
			while (!society.products[pro_id].producible);
			avaEqu_number = ra.nextInt(50) + 1 - ra.nextDouble();
			stocks = round(capital / society.products[pro_id].avePrice * ra.nextDouble());
		}
	}

	void setPrice(Society society, Random ra)  // 制定售价（依据以往销售情况调整售价）
	{  // if(id==0&&difPrice.length>=cpt)
		...
		if (difPrice.length >= cpt)  // 已积累足够数据，依据以往数据调整
			...
			price = society.products[pro_id].avePrice * (ra.nextDouble() * 0.2d + 0.9d);
		difPrice.put(price);
	}

	void finishProduce(Society society, Produce p1)  // 结束已到时间的生产
	{
		stocks += p1.pro_quantity;
		  // System.out.println("cap["+id+"].stocks+="+p1.pro_quantity);
		avaEqu_number += p1.equ_number * (1d - society.products[pro_id].equ_loss);
		society.workers.avaNumber += p1.workers_number;
		  // System.out.println("wor.temIncome+="+p1.salary);
	}

	boolean finishLoan(Society society, Loan lo1)  // 回收已到期的贷款 返回是否回收成功（若无力偿还则回收失败，仅偿还部分，链表项不消失）
	{
		double d1 = Math.min(lo1.value, society.workers.money);
		society.workers.money -= d1;
		lo1.value -= d1;
		capital += d1;
		return lo1.value <= society.workers.money;
	}

	void updateProduces(Society society)  // 倒计时，结束已到时间的生产，产品收入库存，回收设备，计算设备损耗，归还劳动力，支付工人工资
	{
		Produce p1, p2;
		for (p1 = prod; p1 != null; p1 = p1.next) {
			society.workers.temNumber += p1.workers_number;
			society.workers.temIncome += p1.salary / p1.timelength;
			society.workers.money += p1.salary / p1.timelength;  // 工资日结
			p1.time--;
			/* 倒计时 */}
		while (prod != null && prod.time == 0) {
			finishProduce(society, prod);
			prod = prod.next;
		}
		if (prod == null)
			return;
		for (p2 = prod, p1 = prod.next; p1 != null; p2 = p1, p1 = p1.next)
			if (p1.time == 0) {
				finishProduce(society, p1);
				p2.next = p1.next;
			}
	}

	void updateLoans(Society society)  // 倒计时，回收已到期的贷款
	{
		Loan lo1, lo2;
		for (lo1 = loan; lo1 != null; lo1 = lo1.next)
			lo1.time--;
		while (loan != null && loan.time == 0)
			if (finishLoan(society, loan))
				loan = loan.next;
			else  // 无力偿还
				loan.time = 1;
		if (loan == null)
	...
	void invest(Society society, Random ra)  // 投资（借代、投机、再生产）
	{
		double
			d1,   // 工业生产
			d2,   // 借贷
		...
		if (pro_participant) {
			pro1 = society.products[pro_id];
			d1 = Math.pow(pro1.interestRate(society, ra.nextDouble()) + 1d, 1d / pro1.pro_time) - 1d;/* 工业生产单位时间的利率 */
			  // System.out.println("工业："+pro1.interestRate(0.5d));
		} else
			d1 = 0d;
		  // if(id==0)
		  // System.out.print((float)d1+",");
		d6 = Loan.interestRate(society, ra.nextDouble());  // 借贷利率
		d2 = Math.pow(d6 + 1d, 1d / Loan.loanTerm) - 1d;  // 借贷单位时间的利率
														  // （因为过于激进的利率预期可能会<-1，所以可能会得到NaN）
		d3 = Double.NEGATIVE_INFINITY;
		for (i1 = 0; i1 < society.productKinds; i1++)  // 寻找最佳投机产品，计算投机单位时间的利率
		{
			pro2 = society.products[i1];
		...
		if (d4 != 0)  // 有利可图
		{
			...
			{  // System.out.println("工业生产投资："+d1);
				...
				if (d4 > d5)  // 一部分资金用于购置新设备
				{
					...
					// society.workers.money+=i1*pro1.equ_price;  //为了防止货币从系统流出，假定资本家花费的不变资本全部进入工人收入  // TODO: 不知道应不应该加这行
				...
				for (; c * i1 + society.workers.salary(i1 * pro1.workers_number) * i1 * pro1.workers_number
						* pro1.pro_time < d1; i1++)
					;  // 根据劳动力价格调整开动设备数
				for (; (c * i1
						+ society.workers.salary(i1 * pro1.workers_number) * i1 * pro1.workers_number * pro1.pro_time > d1
						|| i1 > avaEqu_number) && i1 > 0; i1--)
					;  // 根据劳动力价格调整开动设备数
				  // System.out.println(" 开动设备数："+i1);
				capital -= c * i1
						+ society.workers.salary(i1 * pro1.workers_number) * i1 * pro1.workers_number * pro1.pro_time;  // 将资本用于进行生产
				society.workers.money += c * i1;  // 为了防止货币从系统流出，假定资本家花费的不变资本全部进入工人收入
				  // System.out.println("c/E="+pro1.c()+"*"+pro1.equ_productivity+"="+c);
				  // System.out.println("wor.temIncome+="+c+"*"+i1+"="+c*i1);
				avaEqu_number -= i1;
				society.workers.avaNumber -= i1 * pro1.workers_number;
				  // System.out.println("雇佣工人数["+id+"]="+i1*pro1.workers_number);
				prod = new Produce(i1 * pro1.equ_productivity, i1, i1 * pro1.workers_number,
						society.workers.salary(i1 * pro1.workers_number), pro1.pro_time, prod);
				/* 头插进行中的生产链表 */}
			...
			{  // System.out.println("借贷投资："+d2);
				d6 = d2 * (1 + d6);
				capital -= d2;
				society.workers.money += d2;
				loan = new Loan(d6, Loan.loanTerm, loan);
				/* 头插贷款（向工人）链表链表 */}
	...
	void print(Society society)  // 打印信息
	{
		...
		if (pro_participant) {
			System.out.println("生产的产品：" + society.products[pro_id].name);
